...
def load_config(FLAGS):
    if FLAGS.tiny:
        ...
        XYSCALE = cfg.YOLO.XYSCALE_TINY if FLAGS.architecture == 'yolov4' else [1, 1]
    else:
        STRIDES = np.array(cfg.YOLO.STRIDES)
        if FLAGS.architecture == 'yolov4':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS, FLAGS.tiny)
        elif FLAGS.architecture == 'yolov3':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS_V3, FLAGS.tiny)
        XYSCALE = cfg.YOLO.XYSCALE if FLAGS.architecture == 'yolov4' else [1, 1, 1]
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if class_name not in allowed_classes:
            ...
            c1, c2 = (np.int(coor[1]), np.int(coor[0])), (np.int(coor[3]), np.int(coor[2]))
            cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)

            if show_label:
                bbox_mess = '%s: %.2f' % (classes[class_ind], score)
                t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
                c3 = (np.int(c1[0] + t_size[0]), np.int(c1[1] - t_size[1] - 3))
